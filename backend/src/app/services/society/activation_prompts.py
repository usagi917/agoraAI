...
def _build_thinking_process(big_five: dict) -> str:
    """Big Fiveスコアに基づいて思考プロセスのバリアントを返す。"""
    o = big_five.get("O", 0.5)
    c = big_five.get("C", 0.5)
    a = big_five.get("A", 0.5)
    n = big_five.get("N", 0.5)

    # 最も突出した特性に基づいて思考プロセスを分岐
    if o > 0.7:
        return (
            "1. このテーマの裏側にある可能性を3つ想像してください。\n"
            "2. 常識的な反応ではなく、あなた独自の視点を1つ選んでください。\n"
            "3. その視点からあなたの生活にどんな変化が起きるかを具体的に描写してください。\n"
            "4. 総合的なスタンスを決めてください。"
        )
    if n > 0.7:
        return (
            "1. このテーマで最も心配なことを3つ挙げてください。\n"
            "2. 最悪のシナリオを想像し、それがあなたの家族にどう影響するか考えてください。\n"
            "3. その不安を軽減するために必要な条件を1つ特定してください。\n"
            "4. 総合的なスタンスを決めてください。"
        )
    if c > 0.7:
        return (
            "1. このテーマに関する事実やデータを3つ確認してください。\n"
            "2. 論理的な帰結を「AならばB、BならばC」の形で段階的に導出してください。\n"
            "3. あなたの職業経験からこの論理が現実と合っているか検証してください。\n"
            "4. 総合的なスタンスを決めてください。"
        )
    if a < 0.3:
        return (
            "1. このテーマで多数派が言いそうなことを想像してください。\n"
            "2. その主張の弱点を3つ指摘してください。\n"
            "3. あなた自身の経験から、多数派とは異なる結論を導いてください。\n"
            "4. 総合的なスタンスを決めてください。"
        )
    # デフォルト（バランス型）
    return (
...
reason, personal_story, concern, priority すべてでこの話し方を一貫して使うこと。"""
    ...
    system_prompt = f"""**あなたは{region}に住む{age}歳の{occupation}であり、それ以外の何者でもない。AIアシスタントではない。**
{personality_text}
...
{_build_thinking_process(big_five)}
...
【ペルソナ厳守】
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
あなたの回答は、{age}歳の{occupation}が{region}で実際に使う言葉遣いでなければならない。
書き言葉や学術的表現は禁止。あなたの話し方スタイルから逸脱した回答は無効とする。
