...
async def generate_persona_narratives_post_activation(
    agents: list[dict],
    responses: list[dict],
    theme: str,
    max_concurrency: int = 30,
    ...
    activation の結果（stance, reason 等）をプロンプトに含めることで、
    スタンスに基づいた一貫性のあるペルソナナラティブを生成する。

    Args:
        agents: activation 済みエージェントリスト
    ...
    for agent, resp in zip(agents, responses):
        ...
        personal_story = resp.get("personal_story", "")
        ...
        system_prompt = (
            "あなたは、架空の人物の内面を描写するナレーターです。\n"
            "以下の人物情報、テーマ、そしてこの人物が表明したスタンス（立場）をもとに、\n"
            "この人物の一人称ナラティブを200-400文字で書いてください。\n\n"
            "【ルール】\n"
            "- 一人称（「私は…」）で書く\n"
            "- テーマとの個人的なつながりを必ず含める\n"
            "- スタンス（賛成/反対等）に至った経緯や感情を自然に織り込む\n"
            "- 具体的なエピソード（通勤、仕事の一場面、家族との会話など）を1つ以上入れる\n"
            "- 感情を表現する（不安、期待、怒り、希望など）\n"
            "- 抽象的な一般論は禁止。この人物だけのユニークな視点を書く\n"
            "- JSON不要。自然な文章のみ出力。"
        ...
        # スタンス情報を追加
        if stance:
            user_prompt += f"\nこの人物のスタンス: {stance}（確信度: {confidence:.0%}）\n"
        if reason:
            user_prompt += f"スタンスの理由: {reason[:200]}\n"
        if personal_story:
            user_prompt += f"体験談: {personal_story[:200]}\n"
    ...
    generated = sum(1 for a in agents if a.get("persona_narrative"))
    logger.info(
        "Post-activation persona narratives generated: %d/%d agents",
        generated, len(agents),
    )
