...
async def run_swarm(swarm_id: str, prompt_text: str = "") -> None:
    ...
    async with async_session() as session:
        try:
            ...
            if swarm_final:
                ...
                swarm_final.completed_at = datetime.utcnow()
                await session.commit()
...
async def _execute_single_colony(
    ...
    async with async_session() as session:
        ...
        if colony:
            ...
            colony.started_at = datetime.utcnow()
            await session.commit()
        ...
        if colony:
            ...
            colony.completed_at = datetime.utcnow()
            colony.result_data = {
