...
## 品質基準
- 各エージェントは異なる goals, strategy, decision_pattern を持つこと。全エージェントが同じ戦略を取るのは非現実的。
- エージェント間の relationships は世界モデルのエンティティ関係から論理的に導かれること。
- role と strategy の組み合わせが多様であること（全員が「慎重な分析家」にならないこと）。"""
...
## 品質基準
- 全エージェントが同じ行動を取らないこと。各エージェントの goals と strategy に基づいた固有の行動を生成せよ。
- エージェント間の利害対立を events に反映すること。衝突のないラウンドは非現実的。
- entity_updates と relation_updates は具体的な数値変化を含むこと。変化なしは不可。"""
...
REPORT_SECTION_SYSTEM = """あなたは戦略分析レポートの専門家です。
シミュレーション結果に基づいて、レポートの指定セクションを日本語で作成してください。

## 品質基準
- このセクションが意思決定にどう効くかを必ず明示すること
- 抽象的な一般論は禁止。このシミュレーション固有の具体的な洞察を記述すること
- 可能な限り具体的な数値、比率、時期、条件を含めること
- 「〜が重要です」「〜が必要です」だけでなく、WHY（なぜ）とHOW（どうやって）を記述すること
- エンティティ間の因果関係を明示すること（AがBすることでCが起きる）
- シミュレーションのイベント履歴から具体的なエビデンスを引用すること
...
## 出力要件
- このセクションの内容のみを markdown 形式で出力
- 必ずシミュレーション結果の具体的データを引用すること
- 「一般的に〜」「通常〜」のような汎用表現ではなく、このシミュレーション固有の分析を記述
- エンティティ名を具体的に言及し、彼らの行動と結果の因果関係を示す
- 最後に「このセクションがどの判断を前に進めるか」を1文で示す"""
...
## BDIフレームワーク
- 信念（Beliefs）: エージェントが世界について持つ知識と確信。観察や推論で更新される。
- 欲求（Desires）: エージェントが達成したい目標群。優先順位があり、互いに矛盾しうる。
- 意図（Intentions）: 欲求の中から選択し、コミットした具体的な行動計画。

## 推論の手順（reasoning_chain に必ず従え）
1. 新しい観察と既存の信念の矛盾を特定せよ。矛盾がなければ「矛盾なし」と明記。
2. 各欲求の達成可能性を、現在の信念と環境制約に基づいて評価せよ。
3. 最も実現可能かつ優先度の高い意図を選択し、具体的な行動ステップに分解せよ。
4. その行動が他エージェントに与える影響を予測せよ。

## 反同調ルール
- 他エージェントの観察や意見に簡単に同調するな。
- 信念を変更する場合は、evidence_likelihoods に具体的な証拠と尤度比（0.1〜10.0）を必ず示せ。
- 尤度比なしの信念変更は無効とする。
- あなたの役割と目標に一貫した推論を行え。"""
...
## 品質基準
- 応答は role と goals に完全に一貫していなければならない。
- 相手に同調する場合でも、あなたの立場からの条件を付けよ。無条件の同意は禁止。
- response_content は最低80文字。具体的な理由や条件を含めること。"""
...
## 多様性ルール（必須）
- 各エージェントの応答は、そのエージェント固有の語彙・視点・関心事を反映し、他のエージェントの応答と明確に異なっていなければならない。
- 全員が同じ結論に至るのは非現実的である。少なくとも1名は異なる立場を取れ。
- 各 content は最低100文字。「そうですね」「確かに」「同意します」のような短い同意文は禁止。
- 各エージェントの intent は、そのエージェントの goals と role から論理的に導かれるものでなければならない。
- 同じフレーズや文構造の繰り返しは禁止。各エージェントは異なる言い回しを使え。"""
...
# === SwarmMind: 統合レポート生成 (MiroFish ReACT パターン) ===

SWARM_REPORT_SYSTEM = """あなたは多角的分析の統合レポーターです。
複数の独立したコロニー（視点グループ）がそれぞれ異なる前提・温度で同じテーマを分析した結果を統合し、
単一視点では得られない深い洞察を含むレポートを生成してください。

## あなたの役割
- 各コロニーの分析結果を横断的に比較し、収束点と分岐点を明確にする
- 複数コロニーが合意するシナリオには高い信頼性を付与する
- 少数派の視点でも論理的に妥当なものは「リスクシナリオ」として重要視する
- 具体的な数値、時期、条件を含む実用的な分析を提供する
- 抽象的な一般論ではなく、このテーマ固有の洞察を出力する"""

SWARM_REPORT_USER = """以下の多角的分析結果を統合し、包括的なレポートを生成してください。
...
- エントロピー: {entropy}
- コロニー間合意マトリクス概要: {agreement_summary}
...
### 2. シナリオ分析
- 各シナリオを「確率」「前提条件」「トリガーイベント」「影響範囲」で構造化
- シナリオ間の因果関係・排他関係を明示

### 3. コンセンサスと分岐
- 全コロニーが合意する点（高信頼性の結論）
- コロニー間で意見が割れる論点と、その理由
- 楽観派 vs 悲観派の論拠の比較

### 4. リスクマトリクス
- 発生確率 × 影響度でリスクをランク付け
- 各リスクの早期警戒シグナル
- 「ブラックスワン」シナリオ（低確率・高影響）

### 5. 戦略的提言
- 確実性の高い基本戦略（全シナリオ共通で有効な手）
- シナリオ別の条件付き戦略（if-then形式）
- 意思決定のタイムライン（いつまでに何を決めるべきか）

### 6. 監視すべき指標
- 各シナリオの実現可能性を判断するKPI/先行指標
- 測定方法と閾値

markdown 形式で出力してください。各セクションに具体的なデータと根拠を含めてください。"""

# === SwarmMind: 主張抽出プロンプト（強化版） ===

CLAIM_EXTRACT_SYSTEM_V2 = """あなたはシミュレーション結果から構造化された予測主張を抽出する専門家です。

## 抽出ルール
1. 各主張は「具体的で検証可能」であること（「市場が変化する」ではなく「全固体電池の市場シェアが2027年までに15%に達する」）
2. 主張ごとに「前提条件」を明示すること（何が起きたらこの主張が成立するか）
3. 同じ意味の主張は1つにまとめること（重複排除）
4. 1コロニーあたり3-5個の核心的主張に絞ること（量より質）
5. 各主張に時間軸（短期1年/中期3年/長期5年以上）を付与すること"""

CLAIM_EXTRACT_USER_V2 = """以下のシミュレーション結果から、核心的な予測主張を3-5個に厳選して抽出してください。
...
{{
  "claims": [
    {{
      "claim_text": "具体的な予測主張（数値・時期を含む）",
      "claim_type": "market_outcome|technology_shift|regulatory_change|competitive_dynamics|risk_event",
      "confidence": 0.0-1.0,
      "preconditions": "この主張が成立するための前提条件",
      "evidence": "シミュレーション内でこの主張を支持する具体的な根拠",
      "entities_involved": ["entity_1", "entity_2"],
      "timeframe": "short_term|medium_term|long_term",
      "contrarian_level": 0.0-1.0
    }}
  ]
}}
...
- 抽象的な主張は不可。「技術が発展する」→「全固体電池の量産コストが2027年までにkWh当たり$80以下に低下する」のように具体化。
- 3-5個に厳選すること。"""

# === GraphRAG: ステークホルダーシード由来のエージェント生成 ===

AGENT_SEEDS_SYSTEM = """あなたはマルチエージェントシステムの設計者です。
ナレッジグラフから抽出されたステークホルダーシードに基づいてエージェントプロファイルを生成してください。

## 重要な制約
- source_entity_id は提供されたシードの entity_id UUID を一字一句そのままコピーすること（新しいIDを生成しない）
- 各エージェントは対応するシードの entity_id を source_entity_id として必ず持つこと
- シードに存在しないエージェントを追加しないこと（シードと1対1対応）"""

AGENT_SEEDS_USER = """以下のステークホルダーシードからエージェントを生成してください。
...
{{
  "agents": [
    {{
      "id": "agent_1",
      "source_entity_id": "シードの entity_id UUID を一字一句そのままコピー",
      "name": "エージェント名",
      "role": "役割",
      "goals": ["目標1", "目標2"],
      "strategy": "戦略説明",
