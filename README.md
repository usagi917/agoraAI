...
- LaunchPad から自由入力、4種類の質問ウィザード、または登録済み分析テンプレートで分析を開始できます。
- API では `quick` / `standard` / `deep` / `research` / `baseline` の5つのプリセットを受け付け、UI では `standard` と `research` を選べます。
...
- `/validate/:id?` では holdout 調査データに対するスタンス分布の予測精度を確認できます。
...
- ユーザーは LaunchPad で質問、質問ウィザード、ファイル、分析モードを選びます。
- バックエンドは API 入力を `quick` / `standard` / `deep` / `research` / `baseline` に正規化し、必要なフェーズだけを実行します。
...
API で利用できるプリセットは次の通りです。LaunchPad の詳細設定では、日常利用向けに `standard` と `research` を選択できます。
...
| holdout 調査に対する検証 API | `backend/src/app/api/routes/validation.py`, `backend/src/app/evaluation/diagnostic.py` |
...
| `GET` | `/validation/topics` | holdout 検証用トピック一覧 |
...
| `GET` | `/simulations/{sim_id}/validation-report` | holdout 調査との分布比較レポート取得 |
...
| `GET` | `/society/simulations/{sim_id}/population-network` | 全人口ネットワーク（描画用コンパクト形式） |
