...
describe('GraphSettingsPanel', () => {
  it('renders sliders for the four physics parameters with Japanese labels', () => {
    const wrapper = mount(GraphSettingsPanel, {
      props: { physics: { ...DEFAULT_PHYSICS } },
    })
    const sliders = wrapper.findAll('input[type="range"]')
    expect(sliders).toHaveLength(4)
    expect(wrapper.text()).toContain('反発力')
    expect(wrapper.text()).toContain('リンク距離')
    expect(wrapper.text()).toContain('中心引力')
    expect(wrapper.text()).toContain('ノード間隔')
  })
